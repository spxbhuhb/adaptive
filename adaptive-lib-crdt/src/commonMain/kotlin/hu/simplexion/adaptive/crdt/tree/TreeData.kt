...
class TreeData(
    ...
    private val activeNodes = Node(FragmentId(0, 1))
    private val deletedNodes = Node(FragmentId(0, 2))

    private val root = Node(FragmentId(0, 0)).also { root ->
    ...
    fun addChildToParent(childID: NodeId, parentID: NodeId) {
        ...
        for ((c, _) in edits) {
            ...
            store.add(TreeOperation(child.id, parent.id, maxCounter + 1, store.tick()))
        }
    }
