...
internal abstract class SyncAdaptiveResourcesForIosTask : DefaultTask() {
    ...
    val xcodeTargetPlatform: Provider<String> =
        providers.gradleProperty("adaptive.ios.resources.platform")
    ...
    val xcodeTargetArchs: Provider<List<String>> =
        providers.gradleProperty("adaptive.ios.resources.archs")
        ...
        platform.startsWith("iphoneos") -> {
            targets.addAll(archs.map { arch ->
                when (arch) {
                    ...
                    // IOS_ARM32 is deprecated "armv7", "armv7s" -> KonanTarget.IOS_ARM32
 ...
 *
 * SyncAdaptiveResourcesForIosTask fails to work with it right now.
 *
 * Gradle attempts to create an output folder for SyncAdaptiveResourcesForIosTask on our behalf,
 * so we can't handle an exception when it occurs. Therefore, we make SyncAdaptiveResourcesForIosTask
 * depend on CheckCanAccessAdaptiveResourcesDirectory, where we check ENABLE_USER_SCRIPT_SANDBOXING.
 */
internal abstract class CheckCanAccessAdaptiveResourcesDirectory : DefaultTask() {
    @get:Input
    ...
    fun run() {
        if (enabled.get()) {
            logger.error("""
                Failed to sync adaptive resources!
