...
private fun ResourceType.getClassName(): ClassName = when (this) {
    ResourceType.DRAWABLE -> ClassName("hu.simplexion.adaptive.resource", "DrawableResource")
    ResourceType.FONT -> ClassName("hu.simplexion.adaptive.resource", "FontResource")
...
private val resourceItemClass = ClassName("hu.simplexion.adaptive.resource", "ResourceItem")
private val experimentalAnnotation = AnnotationSpec.builder(
    ClassName("hu.simplexion.adaptive.resource", "ExperimentalResourceApi")
).build()

private fun CodeBlock.Builder.addQualifiers(resourceItem: ResourceItem): CodeBlock.Builder {
    val languageQualifier = ClassName("hu.simplexion.adaptive.resource", "LanguageQualifier")
    val regionQualifier = ClassName("hu.simplexion.adaptive.resource", "RegionQualifier")
    val themeQualifier = ClassName("hu.simplexion.adaptive.resource", "ThemeQualifier")
    val densityQualifier = ClassName("hu.simplexion.adaptive.resource", "DensityQualifier")
...
//        )
        file.addType(TypeSpec.objectBuilder("Res").also { resObject ->
            ...
            //readFileBytes
            val readResourceBytes = MemberName("hu.simplexion.adaptive.resource", "readResourceBytes")
            ...
            //getUri
            val getResourceUri = MemberName("hu.simplexion.adaptive.resource", "getResourceUri")
            resObject.addFunction(
