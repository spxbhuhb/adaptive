...
class AvValueWorkerTest {
    ...
    fun `should update values when newer timestamp is received`() = test { worker ->
        ...
        val oldValue = AvString(valueId, Instant.parse("2023-01-01T12:00:00Z"), AvStatus.OK, "OldValue")
        val newValue = AvString(valueId, Instant.parse("2023-01-01T12:01:00Z"), AvStatus.OK, "NewValue")
    ...
    fun `should not update values when older timestamp is received`() = test { worker ->
        val valueId = AvValueId()
        val oldValue = AvString(valueId, Instant.parse("2023-01-01T12:01:00Z"), AvStatus.OK, "OldValue")
        val newValue = AvString(valueId, Instant.parse("2023-01-01T12:00:00Z"), AvStatus.OK, "NewValue")
    ...
    fun `should subscribe and receive initial values`() = test { worker ->
        val valueId = AvValueId()
        val initialValue = AvString(valueId, Instant.parse("2023-01-01T12:00:00Z"), AvStatus.OK, "InitialValue")
    ...
    fun `should allow multiple subscribers to receive updates`() = test { worker ->
        ...
        val newValue = AvString(valueId, Instant.parse("2023-01-01T12:02:00Z"), AvStatus.OK, "MultiSubscriberValue")
        worker.queueAdd(newValue)
    ...
    fun `should unsubscribe properly`() = test { worker ->
        ...
        val newValue = AvString(valueId, Instant.parse("2023-01-01T12:03:00Z"), AvStatus.OK, "AfterUnsubscribe")
        worker.queueAdd(newValue)
