...
class FilePersistence(
    ...
    var writeCache = mutableMapOf<AvValueId, AvValue2>()

    val store = object : UuidFileStore<MutableMap<AvValueId, AvValue2>>(root, levels) {

        override fun loadPath(path: Path, map: MutableMap<AvValueId, AvValue2>) {
            ...
            try {
                ...
                check(value is AvValue2) { "Value is not an AvValue" }
    ...
    override fun loadValues(map: MutableMap<AvValueId, AvValue2>) {
        if (useWriteCache) {
            scope.launch {
                while (isActive) {
                    flushCache()
    ...
    override fun saveValue(value: AvValue2) {
        if (! useWriteCache) {
            writeValue(value)
        } else {
            lock.use {
