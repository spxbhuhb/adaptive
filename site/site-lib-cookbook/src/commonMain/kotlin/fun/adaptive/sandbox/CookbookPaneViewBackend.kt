...
internal class CookbookPaneViewBackend(
    ...
    override fun accepts(pane: PaneDef<CookbookPaneViewBackend>, modifiers: Set<EventModifier>, item: WsPaneItem): Boolean {
        if (item !is CbWsRecipeItem) return false
        if (EventModifier.ALT in modifiers) return false
        return true
    }

    override fun load(pane: PaneDef<CookbookPaneViewBackend>, modifiers: Set<EventModifier>, item: WsPaneItem): PaneDef<CookbookPaneViewBackend> {
