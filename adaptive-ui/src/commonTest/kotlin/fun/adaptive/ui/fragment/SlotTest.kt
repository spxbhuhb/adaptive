...
import `fun`.adaptive.foundation.testing.AdaptiveT0
...
class SlotTest {
    ...
    fun setup(adapter: AuiTestAdapter): AuiSlot {
        FoundationDelegate(
            adapter, null, 0,

            buildFun = { parent, index ->
                when (index) {
                    SLOT -> AuiSlot(parent.adapter as AbstractAuiAdapter<*, *>, parent, index)
