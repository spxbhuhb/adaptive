...
class Workspace {
    ...
    val toolPanes = mutableListOf<WorkspacePane>()

    val contentPaneGroups = mutableListOf<WorkspaceContentPaneGroup>()
    ...
    fun directPanes() =
        toolPanes.filter { it.direct }

    fun topPanes(left: Boolean) =
        toolPanes.filter(if (left) WorkspacePanePosition.LeftTop else WorkspacePanePosition.RightTop)

    fun middlePanes(left: Boolean) =
        toolPanes.filter(if (left) WorkspacePanePosition.LeftMiddle else WorkspacePanePosition.RightMiddle)

    fun bottomPanes(left: Boolean) =
        toolPanes.filter(if (left) WorkspacePanePosition.LeftBottom else WorkspacePanePosition.RightBottom)
